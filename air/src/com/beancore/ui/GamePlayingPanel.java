...
public class GamePlayingPanel extends JPanel implements MouseListener,
		MouseMotionListener, BulletListener, EnemyPlaneListener,
		CatchableWeaponListener, ImageObserver {
	...
	public GamePlayingPanel() throws LineUnavailableException,
	...
	private void initSoundPlayer() throws LineUnavailableException,
			UnsupportedAudioFileException, IOException {
		smallPlaneKilledSoundPlayer = new SoundPlayer(
				Config.SMALL_PLANE_KILLED_AUDIO);
		bigPlaneKilledSoundPlayer = new SoundPlayer(
				Config.BIG_PLANE_KILLED_AUDIO);
		bossPlaneKilledSoundPlayer = new SoundPlayer(
				Config.BOSS_PLANE_KILLED_AUDIO);

		bossPlaneFlyingSoundPlayer = new SoundPlayer(
				Config.BOSS_PLANE_FLYING_AUDIO);
		...
		getDoubleLaserSoundPlayer = new SoundPlayer(
				Config.GET_DOUBLE_LASER_AUDIO);
	...
	public void onBulletLocationChanged(Bullet b) {
		if (b != null) {
			...
			if (enemyPlane != null) {
				enemyPlane.drawFighting(this.getComponentGraphics(this
						.getGraphics()));
				if (enemyPlane.isKilled()) {
					...
					enemyPlane.drawKilled(this.getComponentGraphics(this
							.getGraphics()));
				}
			}
		}
	...
	public void onCatchableWeaponLocationChanged(CatchableWeapon weapon) {
		if (weapon != null) {
			synchronized (weapon) {
				...
				if (!weapon.isUseAnimationDone()
						&& weapon.getPosY() >= this.getHeight() / 5) {
	...
	private void drawBomb(Graphics g) {
		if (this.myPlane.getHoldBombCount() > 0) {
			...
			g2d.drawImage(Images.CAUGHT_BOMB_IMG, posX, posY,
			...
			g2d.drawImage(Images.X_MARK_IMG, posX, posY,
		...
		public void run() {
			while (myPlane.isAlive()) {
				...
				if (remainTimeToPopSmallPlane > 0) {
					...
					EnemyPlane smallPlane = EnemyPlaneFactory.createEnemyPlane(
							GamePlayingPanel.this,
				...
				if (remainTimeToPopBigPlane > 0) {
					...
					EnemyPlane bigPlane = EnemyPlaneFactory.createEnemyPlane(
							GamePlayingPanel.this,
				...
				if (remainTimeToPopBossPlane > 0) {
					...
					EnemyPlane bossPlane = EnemyPlaneFactory.createEnemyPlane(
							GamePlayingPanel.this,
				...
				if (remainTimeToPopBomb > 0) {
					...
					// pop bomb
					popBomb = CatchableWeaponFactory.createCatchableWeapon(
							GamePlayingPanel.this, CatchableWeaponType.BOMB);
				...
				if (remainTimeToPopDoubleLaser > 0) {
					...
					// pop double laser
					popDoubleLaser = CatchableWeaponFactory
							.createCatchableWeapon(GamePlayingPanel.this,
				...
				if (popDoubleLaser != null
						&& popDoubleLaser.isWeaponDisappear()) {
					popDoubleLaser = null;
				}
	...
	protected void paintComponent(Graphics g) {
		...
		if (this.popDoubleLaser != null
				&& !this.popDoubleLaser.isWeaponDisappear()) {
	...
	public void startGame() {
		...
		this.myPlane
				.setPosX((Config.MAIN_FRAME_WIDTH - ImageConstants.MY_PLANE_WIDTH) / 2);
		this.myPlane.setPosY(Config.MAIN_FRAME_HEIGHT
